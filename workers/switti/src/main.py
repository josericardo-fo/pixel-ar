...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - [SWITTI] - %(levelname)s - %(message)s"
)
...
try:
    ...
    try:
        ...
        logger.critical(
            "ERRO: Não foi possível importar 'SwittiPipeline'. Verifique se 'models/switti' foi copiado corretamente."
        )
        exit(1)
